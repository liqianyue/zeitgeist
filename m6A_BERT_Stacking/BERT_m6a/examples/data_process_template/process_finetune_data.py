...
def Find_train(args):
    ...
    write_file(train_lines, args.file_path+"/train_pri.tsv", args.kmer, head=False)
    ...
    for kmer in range(3,7):
        ...
        train_file =  open(os.path.join(args.file_path,"train_pri.tsv"), "r", encoding="utf-8-sig")
        lines = list(csv.reader(train_file, delimiter="\t", quotechar=None))
        train_path = os.path.join(root_path,"train_pri.tsv")
...
def Process_1000(args):
    ...
    # write_file(train_lines, output_path+"/train_pri.tsv", args.kmer, head=False, seq_index=8, label_index=6)
...
def Process_1000_kmer(args, test_lines=None, train_lines=None, tata_test_lines=None, tata_train_lines=None, notata_test_lines=None, notata_train_lines=None):
    ...
    if test_lines == None:
        ...
        path2 = os.path.join(args.file_path,"train_pri.tsv")
    ...
    for kmer in range(3,7):
        ...
        print("writing train")
        write_file(train_lines, os.path.join(all_path,"train_pri.tsv"), kmer, head=False, seq_index=seq_index, label_index=label_index)
        ...
        # write_file(tata_train_lines, os.path.join(tata_path,"train_pri.tsv"), kmer, head=False, seq_index=seq_index, label_index=label_index)
        ...
        write_file(notata_train_lines, os.path.join(notata_path,"train_pri.tsv"), kmer, head=False, seq_index=seq_index, label_index=label_index)
...
def Process_splice(args):
    ...
    #     root_path = os.path.join(args.file_path, str(kmer))
    #     os.makedirs(root_path)
    #     f_train = open(os.path.join(root_path, "train_pri.tsv"), "wt")
...
def Process_prom_core(args):
    ...
    for kmer in range(3,7):
        ...
        write_file(train_lines, os.path.join(root_path,"train_pri.tsv"), kmer, head=False, seq_index=1, label_index=2)
...
def Process_pair(args):
    ...
    f_train = open(os.path.join(output_path, "train_pri.tsv"), 'wt')
...
def Process_p53(args):
    ...
    for kmer in range(3, 7):
        ...
        write_file(train_lines, os.path.join(output_path, "train_pri.tsv"), kmer, head=True, seq_index=2, label_index=3)
        if args.dev:
...
def Generate_prom_train_dev(args):
    ...
    write_file(train_lines, args.file_path+"/train_pri.tsv", args.kmer)
...
def Process_mouse(args):
    ...
    for i in range(num_task):
        ...
        #     write_file(train_lines, os.path.join(args.file_path, str(kmer), index, "train_pri.tsv"), kmer, head=True, seq_index=2, label_index=3)
