...
def predict(args, model, tokenizer, prefix=""):
    ...
    for pred_task, pred_output_dir in zip(pred_task_names, pred_outputs_dirs):
        ...
        if args.output_mode == "classification":
            if args.task_name[:3] == "dna" and args.task_name != "dnasplice":
                if args.do_ensemble_pred or pre_pro_all:
                    probs = softmax(torch.tensor(preds, dtype=torch.float32)).numpy()

                    # COOK
                    out_probs = (torch.tensor(preds, dtype=torch.float32)).numpy()
        ...
        np.save(output_pred_file, out_probs)
...
def load_and_cache_examples(args, task, tokenizer, evaluate=True):
