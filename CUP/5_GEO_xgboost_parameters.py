...
GEO_label=list(GEO_label.loc[:,0])
 ...
 stra_sam_rate=0.2
 GEO_fold_loc=str_sam_get_5(GEO_label,stra_sam_rate,10)
...
clf_xg=XGBClassifier(gamma=ga,max_depth=ma_num,min_child_weight=mi_num,
                     learning_rate=0.1,booster='gbtree',n_jobs=-1)
clf_xg.fit(X_train,y_train)



import pickle
with open("./model_file/GEO_clf_xg_all.pickle",'wb') as f:
    pickle.dump(clf_xg,f)


#stander the testing set
GEO_test=pd.read_csv(loc_path+"GEO_dataset/gene_sel_data/test.csv",header=None)
GEO_test=GEO_test.values
print(GEO_test.shape)

GEO_test=std.transform(GEO_test)
GEO_test=pd.DataFrame(GEO_test)
GEO_test.to_csv(loc_path+"GEO_dataset/gene_sel_data/test_standard.csv",header=None,index=0)
